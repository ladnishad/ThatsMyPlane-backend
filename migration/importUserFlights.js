...
export const ImportUserFlights = async () => {
  ...
  const processedFlights = await asyncMap(
    MyFlightsInUSA,
    async ({
      airline,
      aircraftType,
      ...
      try {
        const airlineOnDb = await Airline.findOne({
          name: airline.toUpperCase(),
        }).exec();
        ...
        const originAirport = await Airport.findOne({
          IATA: flightOriginAirport,
        }).exec();

        const destinationAirport = await Airport.findOne({
          IATA: flightDestinationAirport,
        }).exec();
        ...
        const aircraftTypeOnDb = await AircraftType.findOne({
          ICAO: aircraftType,
        });
        ...
        const scheduledOut =
          flightDate === "" ? 0 : dayjs(flightDate).valueOf();
  ...
  console.log(
    `Data processing complete with data of length ${processedFlights?.length}`
  );
  ...
  try {
    ...
    await asyncForEach(processedFlights, async (eachFlight) => {
      const addedUserFlight = await FlightSetters.addUserFlight({
        userId: user?._id,
        userFlight: eachFlight,
        caption: "",
        visibility: "Private",
      });
